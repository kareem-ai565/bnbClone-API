...
namespace bnbClone_API.Controllers
{
    ...
    public class HostVerificationController : ControllerBase
    {
        ...
        // Original endpoint that returns basic data
        [HttpGet]
        //[Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetAllVerifications()
        {
        ...
        // Enhanced endpoint that returns data with document URLs
        [HttpGet("with-documents")]
        //[Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetAllVerificationsWithDocuments()
        {
            try
            {
                var hostVerifications = await _ihostVerification.GetAllHostVerificationWithDocuments();

                if (hostVerifications.Any())
                {
                    return Ok(hostVerifications);
                }
                else
        ...
        // Enhanced endpoint with documents
        [HttpGet("{id}/with-documents")]
        //[Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetVerificationByIdWithDocuments(int id)
        {
            try
            {
                var verification = await _ihostVerification.GetHostVerificationByIdWithDocuments(id);
                return Ok(verification);
            }
            catch (KeyNotFoundException ex)
            {
        ...
        // Endpoint to serve individual document files
        [HttpGet("{id}/document/{documentNumber}")]
        //[Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetDocumentFile(int id, int documentNumber)
        {
            try
            {
                if (documentNumber != 1 && documentNumber != 2)
                {
                    return BadRequest(new { error = "Document number must be 1 or 2" });
                }

                var (fileBytes, contentType, fileName) = await _ihostVerification.GetDocumentFile(id, documentNumber);
        ...
        // Enhanced endpoint with documents
        [HttpGet("status/{status}/with-documents")]
        //[Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetVerificationsByStatusWithDocuments(string status)
        {
            try
            {
                var verifications = await _ihostVerification.GetHostVerificationsByStatusWithDocuments(status);
        ...
        // Enhanced endpoint with documents
        [HttpGet("host/{hostId}/with-documents")]
        //[Authorize(Roles = "Admin")]
        public async Task<IActionResult> GetVerificationsByHostIdWithDocuments(int hostId)
        {
            try
            {
                var verifications = await _ihostVerification.GetHostVerificationsByHostIdWithDocuments(hostId);
